class Robot
  ...
  def initialize(name)
    @position         = BotTrustHelper::INITIAL_STARTING_POSITION
    @name             = name
    @buttons_to_press = []
    @buttons_pressed  = []
  ...
  def move_forward
    @position += BotTrustHelper::POSITION_MOVE_DISTANCE unless @position == BotTrustHelper::FINAL_ENDING_POSITION
  end

  def move_backward
    @position -= BotTrustHelper::POSITION_MOVE_DISTANCE unless @position == BotTrustHelper::INITIAL_STARTING_POSITION
  end
